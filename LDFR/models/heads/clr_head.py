...
class CLRHead(nn.Module):
    def __init__(self,
        ...
        self.query = nn.Embedding(192,64)
        self.input_proj = nn.Conv2d(64, 64, kernel_size=1)
        self.roi_gather = ROIGather(self.prior_feat_channels, self.num_priors,
                                    self.sample_points, self.fc_hidden_dim,
                                    self.refine_layers)
        self.class_embed = nn.Linear(64, 2)
        self.bbox_embed = build_MLP(64, 64, 3, 2)
        self.reg_layers = nn.Linear(
            self.fc_hidden_dim, self.n_offsets)  # n offsets + 1 length + start_x + start_y + theta + 1 + 2 + 1
        self.cls_layers = nn.Linear(self.fc_hidden_dim, 2)
        self.len_layers = nn.Linear(self.fc_hidden_dim,1)
        self.the_layers = nn.Linear(self.fc_hidden_dim,1)

        #self.head_ctr =
        self.sta_layers = build_MLP(1000,self.fc_hidden_dim,2,2)
    ...
    def generate_priors_from_embeddings(self):
        ...
        # pa= torch.cos(priors[:, 4].clone() * math.pi + 1e-5)
        # length = (priors[:,-1] - priors[:,3])
        # priors[:,5] = length/pa
    ...
    def forward(self, x, **kwargs):
        ...
        for stage in range(self.refine_layers):
            # src = batch_features[stage]
            # mask = None
            # hs = self.transformer(self.input_proj(src),mask,self.query.weight,pos)[0].squeeze(0)
           ...
           # xyfea = self.ctrHead(batch_features[stage])
            ...
            length = self.len_layers(reg_features)
            xyt = self.bbox_embed(reg_features)
            #theta = self.the_layers(reg_features)
            #xy = self.sta_layers(xyfea)

            cls_logits = cls_logits.reshape(
                batch_size, -1, cls_logits.shape[1])  # (B, num_priors, 2)
            reg = reg.reshape(batch_size, -1, reg.shape[1])
            length = length.reshape(batch_size, -1, length.shape[1])
            xyt = xyt.reshape(batch_size, -1, xyt.shape[1])
            #theta = theta.reshape(batch_size, -1, theta.shape[1])
            #xy = xy.reshape(batch_size, -1, xy.shape[1])

            predictions = priors.clone()
            predictions[:, :, :2] = cls_logits
            #predictions[:, :, 2] = length.squeeze(-1)
            # predictions[:, :,
            #              3:5] += xy
            # predictions[:, :, 5] = theta.squeeze(-1)

            predictions[:, :,
                        2:5] += xyt[:, :, :3]  # also reg theta angle here
            predictions[:, :, 5] = length.squeeze(-1)  # length

...
class FPN(nn.Module):
    def __init__(self,
        ...
        self.transformer = TransConvEncoderModule(in_dim=512,
            attn_in_dims=[512, 256],
            attn_out_dims=[256, 64],
            strides = [1, 1],
            ratios=[4, 4],
            pos_shape=(8, 10, 25))
        if end_level == -1:
        ...
        self.copy = nn.ModuleList()
        self.ALU = nn.ModuleList()
        for i in range(self.start_level, self.backbone_end_level):
            alu = RESA(in_channels[i],40//(2**i),100//(2**i))
            l_conv = ConvModule(
                in_channels[i],
                out_channels,
                1,
    ...
    def forward(self, inputs):
        ...
        if len(inputs) > len(self.in_channels):
            for _ in range(len(inputs) - len(self.in_channels)):
                del inputs[0]
        # member = self.transformer(inputs[-1])
        # inputs = [
        #     lateral_conv(inputs[i + self.start_level])
        #     for i, lateral_conv in enumerate(self.ALU)
        # ]
        # build laterals
        laterals = [
            lateral_conv(inputs[i + self.start_level])
